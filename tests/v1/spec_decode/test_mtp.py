...
def test_gpu_runner_packs_compact_sync_mtp_draft_probs_for_decode_only_rows():
    vocab_size = 5
    draft_probs = torch.arange(
        1 * 2 * vocab_size, dtype=torch.float32, device=DEVICE_TYPE
    ).view(1, 2, vocab_size)
    metadata = SpecDecodeMetadata.make_dummy(
        draft_token_ids=[[1, 2], []], device=torch.device(DEVICE_TYPE)
    )
    runner_stub = mock.MagicMock()
    runner_stub._draft_probs = draft_probs
    runner_stub.prev_positions.np = np.full(2, -1)
    runner_stub.use_async_scheduling = False

    packed = GPUModelRunner._get_draft_probs_for_rejection(runner_stub, metadata)

    expected = draft_probs[0, :2]
    assert torch.equal(packed, expected)
    assert packed.is_contiguous()
